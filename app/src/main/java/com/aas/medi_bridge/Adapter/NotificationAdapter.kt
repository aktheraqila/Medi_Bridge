...
class NotificationAdapter(
    private val notifications: MutableList<AppointmentNotification>,
    private val onDeleteNotification: (AppointmentNotification, Int) -> Unit
) : RecyclerView.Adapter<NotificationAdapter.NotificationViewHolder>() {

    inner class NotificationViewHolder(val binding: ViewholderNotificationBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(notification: AppointmentNotification) {
            binding.apply {
                // Use your existing layout element names
                tvDoctorName.text = "${notification.doctorName}"
                tvAppointmentDate.text = notification.appointmentDate
                tvAppointmentTime.text = notification.appointmentTime
                tvPatientName.text = "Patient: ${notification.patientName}"
                ...
                // Format timestamp if notification has one
                if (notification.timestamp > 0) {
                    val date = Date(notification.timestamp)
                    val formatter = SimpleDateFormat("MMM dd, yyyy HH:mm", Locale.getDefault())
                    // If you have a timestamp view, uncomment this line:
    ...
    override fun onBindViewHolder(holder: NotificationViewHolder, position: Int) {
        ...
        holder.binding.btnDelete.setOnClickListener {
            val pos = holder.adapterPosition
            if (pos != RecyclerView.NO_POSITION) {
                onDeleteNotification(notification, pos)
            }
        }
    }
